...
| 分支 | 状态 | 说明 |
|------|------|------|
| `master` | 稳定 | 生产环境使用的稳定版本 |
| `feature/v2-new` | 开发中 | 包含所有最新功能开发和优化 |

---

### 12.2 最近一周提交记录 (2026-02-15 ~ 2026-02-22)

| 提交Hash | 日期 | 功能 | 说明 |
|----------|------|------|------|
| `d51b27e` | 2026-02-22 | RAG 增强 | 集成 LlamaIndex 增强 RAG |
| `cc7b729` | 2026-02-22 | 管理员API | 添加 RAG 管理员 API 端点 |
| `933481f` | 2026-02-22 | RAG 增强 | 增强检索元数据返回与测试体系 |
| `eb06589` | 2026-02-21 | 流式响应 | 实现流式响应主动停止与资源释放 |
| `84e7307` | 2026-02-21 | 可观测性 | 增强 AI 服务可观测性与并发处理 |
| `3884688` | 2026-02-20 | 存储 | 调整内存缓存 TTL 为 6 小时 |
| `3f9408f` | 2026-02-20 | 并发 | 实现多用户并发请求处理 |
| `7275976` | 2026-02-19 | 可观测性 | 集成 Langfuse 实现 AI 调用可观测性 |

---

### 12.3 新增功能详解

#### 12.3.1 RAG 检索增强 (提交: 933481f, cc7b729)

**新增方法** (`vector_store.py`)：

```python
def get_documents_with_scores(self, query: str, k: int = 8, sources: List[str] = None) -> Tuple[List[Document], List[float]]:
    """获取带相似度分数的检索结果"""
    # 直接调用 Milvus API
    # 自动检测索引 metric_type (L2/IP)
```
...
```json
{
  ...
  "sources": [{
    ...
    "max_score": 0.95,
    "avg_score": 0.87,
    "chunks": [{"excerpt": "...", "score": 0.95, "text_length": 500}]
  }],
  "retrieval_metadata": {
    "total_chunks_retrieved": 8,
    "unique_sources_count": 1,
    "score_range": {"min": 0.8, "max": 0.95, "avg": 0.87}
  }
}
```
...
#### 12.3.2 管理员 API 端点 (提交: cc7b729)

| API | 方法 | 功能 |
|-----|------|------|
| `/admin/rag/stats` | GET | RAG 综合统计 |
| `/admin/rag/sources` | GET | 文档源及选中状态 |
| `/admin/rag/sources/select` | POST | 批量选择文档 |
| `/admin/rag/sources/select-all` | POST | 全选所有文档 |
| `/admin/rag/sources/deselect-all` | POST | 取消全选 |
| `/admin/conversations` | GET | 所有会话列表 |
| `/admin/conversations/{id}/messages` | GET | 会话消息历史 |
...
| API | 方法 | 功能 |
|-----|------|------|
| `/test/rag` | GET | 测试 RAG 检索 |
| `/test/vector-stats` | GET | 向量库统计 |
...
| 文件 | 说明 |
|------|------|
| `tests/test_rag_system.py` | pytest 测试套件 (12 个测试) |
| `tests/test_runner.py` | 快速测试运行器 |

---

### 12.5 当前系统状态 (2026-02-22)
...
```
PostgreSQL 数据库
├── conversations (chat_id, messages, message_count)
└── chat_metadata (chat_id, name, created_at)
```
...
|------|------|---------|
| 2026-02-22 | 1.0 | 初始版本 |
| 2026-02-22 | 1.1 | 添加第十二章 - 2026年2月开发更新 |
